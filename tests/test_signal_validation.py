...
def test_signal_id_format(validator: _SignalValidator) -> None:
    ...
    for sid in valid_ids:
        assert validator.validate_signal_id(sid), (
            f"Идентификатор '{sid}' должен пройти валидацию"
        )
    ...
    for sid in invalid_ids:
        assert not validator.validate_signal_id(sid), (
...
def test_risk_filter_blocks_low_edge(validator: _SignalValidator) -> None:
    ...
    assert validator.passes_edge_filter(edge_pct=1.5) is False, (
        "Edge 1.5% < 2.0% должен быть заблокирован"
    )
    assert validator.passes_edge_filter(edge_pct=0.0) is False, (
        "Edge 0.0% < 2.0% должен быть заблокирован"
    )
    assert validator.passes_edge_filter(edge_pct=-1.0) is False, (
        "Отрицательный edge должен быть заблокирован"
    )

    # Граничный случай: ровно на пороге
    assert validator.passes_edge_filter(edge_pct=2.0) is True, (
        "Edge 2.0% == min_edge_pct=2.0% должен пройти (>= включает границу)"
    )

    # Выше порога → пропускаем
    assert validator.passes_edge_filter(edge_pct=3.0) is True, (
        "Edge 3.0% > 2.0% должен пройти фильтр"
    )
    assert validator.passes_edge_filter(edge_pct=10.0) is True, (
...
def test_risk_filter_blocks_high_margin(validator: _SignalValidator) -> None:
    ...
    assert validator.passes_margin_filter(book_margin_pct=9.0) is False, (
        "Маржа 9.0% > 8.0% должна быть заблокирована"
    )
    assert validator.passes_margin_filter(book_margin_pct=15.0) is False, (
        "Маржа 15.0% > 8.0% должна быть заблокирована"
    )

    # Граничный случай
    assert validator.passes_margin_filter(book_margin_pct=8.0) is True, (
        "Маржа 8.0% == max_book_margin_pct должна пройти (включая границу)"
    )

    # Ниже максимума → пропускаем
    assert validator.passes_margin_filter(book_margin_pct=5.0) is True, (
        "Маржа 5.0% < 8.0% должна пройти фильтр"
    )
    assert validator.passes_margin_filter(book_margin_pct=0.0) is True, (
...
def test_paper_ledger_add_and_update(
    ...
    assert abs(record_closed["pnl_units"] - expected_pnl) < 1e-6, (
    ...
    assert abs(record_closed["clv_pct"] - expected_clv) < 0.001, (
    ...
    assert abs(roi - expected_roi) < 0.01, (
        f"ROI={roi:.2f}%, ожидалось {expected_roi:.2f}%"
    )
    ...
    assert abs(loss_record["pnl_units"] - expected_loss_pnl) < 1e-6, (
...
def test_signal_status_always_paper(
    ...
    assert status == "paper", (
        f"Статус сигнала должен быть 'paper', получено '{status}'"
    )

    # Проверяем, что статус в sample_signal корректен
    assert sample_signal["status"] == "paper", (
    ...
    for forbidden in forbidden_statuses:
        assert forbidden != status, (
            f"Статус '{forbidden}' недопустим в MVP-режиме paper trading"
        )
